#' WAIC Monotonic
#'
#' @description Calculates the WAIC value for the Monotonic TRP model
#'
#' @param out Output of Monotonic model fit
...
#' @examples mono.waic.analog <- waic_mono(mono.mcmc, volnames, 3, 1000)
#' @examples mono.waic.target <- waic_mono(mono.mcmc, volnames, 3, 1000, target = TRUE)

waic_mono <- function(out, volnames, minVEI, period, target = F){
  ...
  for (i in 1:N){
    ...
    for (j in 1:length(occur)){
      occurT[,j] <- Psi_mono(occur[j], mu[,i], kappa[,i], cens)
    }
    ...
    # Censored time
    censored <- Psi_mono(cens, mu[,i], kappa[,i], cens) - Psi_mono(occurT[,length(occur)], mu[,i], kappa[,i], cens)
