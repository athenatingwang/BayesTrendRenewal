#' Fit Gamma RP model to data
#'
#' @description Uses Bayesian methods to fit a Gamma renewal process model to a set of volcanoes
#'
...
#'
#' @return Posterior distributions for the parameters of the Gamma RP model (mcmc object)
#' @export
#'
#' @examples ## Fit Gamma model to Taranaki (Target) and analogue volcanoes
...
gamRP_fit <- function(volNames, VEI, period, chains = 3, chainLen, adapt){
  ...
  m <- jags.model(system.file("/Gam_RP_Mod.txt", package = "BayesTrendRenewal"), data = list(nVol = nVol, x = dat$data[,2] / 1000, n1 = dat$start, n2 = dat$finish, censLimVec = dat$data[,4] / 1000, isCensored = dat$data[,3]),
